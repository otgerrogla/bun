...
function expectToThrowAndMatchObject(fn, obj) {
...
test("order of option and positional does not matter (per README)", () => {
  ...
  expect(result).toEqual(expected); // Error('option then positional')
  result = parseArgs({ allowPositionals: true, args: args2, options });
  expect(result).toEqual(expected); // Error('positional then option')
});
...
test("null prototype: when no options then values.toString is undefined", () => {
  ...
  expect(result.values.toString).toEqual(undefined);
});
