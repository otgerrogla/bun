...
test("dash: when args include '-' used as positional then result has '-' in positionals", () => {
  const args = ["-"];
...
test("dash: when args include '-' used as space-separated option value then result has '-' in option value", () => {
  const args = ["-v", "-"];
